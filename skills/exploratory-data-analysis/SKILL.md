...
**Role in the ds plugin:** This skill is invoked by `/ds:eda` at step 2 (File Type Detection). For tabular data (CSV, Parquet, Excel), it identifies the format and hands off to the `data-profiler` agent and `eda-checklist` skill. For non-tabular scientific formats (HDF5, FASTA, PDB, NetCDF, etc.), it runs the full format-specific analysis workflow using its reference files.
...
When invoked by the `/ds:eda` command, save the report to the ds plugin's output directory:
- Pattern: `docs/ds/eda/YYYY-MM-DD-<dataset-name>-eda.md`
- Example: `experiment_data.fastq` -> `docs/ds/eda/2026-02-24-experiment-data-eda.md`
- Create the directory if needed: `mkdir -p docs/ds/eda/`

When used standalone (outside `/ds:eda`), save alongside the source file:
