...
The SHAP skill provides the model interpretability API for the ds plugin. It is the reference for computing Shapley-value-based feature attributions, selecting the right explainer for any model type, and generating interpretability visualizations.

**Boundary with other skills:**
- **scikit-learn** provides `permutation_importance` and tree-based `feature_importances_` -- these are quick, built-in methods. Use SHAP when you need: (a) model-agnostic explanations for any estimator, (b) per-prediction local explanations (waterfall plots), (c) feature interaction detection (scatter plots), or (d) fairness analysis via cohort comparison.
- **statsmodels** provides coefficient-based interpretability (odds ratios, marginal effects, p-values). Use statsmodels when the goal is statistical inference on model coefficients. Use SHAP when interpreting black-box or tree-based models where coefficients are not available.
- **matplotlib** is used by SHAP's plotting API under the hood. For standard SHAP visualizations (`shap.plots.beeswarm()`, `shap.plots.waterfall()`), use SHAP's built-in plots directly. Use matplotlib for customizing SHAP plots, composing them into multi-panel figures, or saving with custom styling.

**DS plugin conventions:**
- Use `shap.Explainer(model)` as the default entry point (auto-selects the best algorithm based on model type)
- For tree-based models (XGBoost, LightGBM, Random Forest, scikit-learn trees): prefer `shap.TreeExplainer` for speed and exactness
- Default to `shap.plots.beeswarm()` for global feature importance and `shap.plots.waterfall()` for single-prediction explanations
- Save SHAP plots with `plt.savefig()` + `plt.close(fig)` -- SHAP plots use matplotlib under the hood. Never call `plt.show()` (headless compatibility)
- For large datasets (>10K rows), compute SHAP values on a sample: `shap_values = explainer(X_test[:1000])`
- For `KernelExplainer`, use `shap.sample(X_train, 100)` as background data to balance accuracy and speed
